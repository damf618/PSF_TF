...
fig    = plt.figure ( 1 )
fig.suptitle('TP_MODULO1 Antialias digital + downsampling con CIAA', fontsize=16)

#--------------------------ADC--------------------------
adcAxe = fig.add_subplot ( 2,1,1                  )
adcLn, = plt.plot        ( [],[],'r-o',linewidth=1,label  = "adc")
adcAxe.grid              ( True                   )
...
fftLn,     = plt.plot ( [] ,[] ,'r-o' ,linewidth = 1  ,label = "abs(FFT(adc))" )
...
def readSamples(adc,synth,N,trigger=False,th=0):
    ...
    i=0
    for t in range(N):
        sample = (readInt4File(streamFile,sign = True)*1.65)/(2**6*512)#/2**6normal *2**4segment /2**2envolvente 
        state,nextI= {
...
def update(t):
    ...
    #adcAxe.set_xlim ( 1000,1100 )#np.max(absFft))
